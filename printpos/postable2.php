...
<div class="ticket" style="width:100%;">
  ...
  <?php                 
   ...
   <table class="table table-stripped"style="width:100%;">
    ...
    <?php
          $name = $_SESSION['name'];          
 $sql = "SELECT id, invoice_id, specie,username, quantity, unit_price, (quantity *unit_price) AS cost FROM joe.temp_sale WHERE username='$name'";
  $result = $connect->query($sql);
  if ($result-> num_rows >0) {
    while ($row = $result-> fetch_assoc()) {?>
      ...
      <tr>
          ...
          ?>" class="btn btn-danger" id="delete">Remove</a></td>
            ...
            $sql = "SELECT id, quantity, unit_price, SUM(quantity*unit_price) AS Amount, SUM(quantity)AS total_quantity FROM joe.temp_sale WHERE username='$name'";
            $result = $connect->query($sql);
            if ($result-> num_rows >0) {
              while ($row = $result-> fetch_assoc()) {?>
