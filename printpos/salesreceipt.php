...
<html lang="en">
 ...
 $sql = "SELECT invoice_id,transaction_date FROM temp_sale LIMIT 1";
...
<?php
  ...
  ?>
            ...
            <table style="text-align: center;">
                    ...
                    <?php
                    include 'db_connect.php';
 $sql = "SELECT invoice_id, product, quantity, unit_price, (quantity *unit_price) AS selling_price, payment_method, transaction_date FROM temp_sale";
  $result = $connect->query($sql);
  if ($result-> num_rows >0) {
    while ($row = $result-> fetch_assoc()) {?>
      ...
      <tr>
          ...
          <td class="description"><?php echo $row['selling_price']; ?></td>
             ...
             <?php 
             include 'db_connect.php';
            $sql = "SELECT quantity, unit_price, SUM(quantity*unit_price) AS Amount, SUM(quantity)AS total_quantity FROM temp_sale";
            $result = $connect->query($sql);
            if ($result-> num_rows >0) {
              while ($row = $result-> fetch_assoc()) {?>
