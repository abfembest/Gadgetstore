...
//if(!empty($card_no)||(!empty($firstname)||(!empty($mothername)||!empty($pphone)||!empty($age)||!empty($dob)||(!empty($teacher)){
	...
	include 'db_connect.php';
		$sql ="INSERT INTO joe.expenses (title, amount, receiver, description, payment_method, username ,transaction_date) VALUES ('$title', '$amount', '$receiver', '$description', '$payment_method', '$username', Current_timestamp );INSERT INTO sales (product, expense_amount, transaction_date) 
		VALUES ( '$product','$amount', current_timestamp);";
		if(mysqli_multi_query($connect, $sql)){

   			echo '<script type="text/javascript">'; 
		echo 'alert("Expense was captured, click ok to continue.");'; 
		echo 'window.location.href = "../expenses.php";';
		echo '</script>';
} 
